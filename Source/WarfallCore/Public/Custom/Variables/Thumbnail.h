...
struct FThumbnail
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TSoftObjectPtr<UTexture2D> Layer;
...
public:
	...
	UMaterialInstanceDynamic* CreateMaterial()
	{
		...
		UTexture2D* LoadThumbnail = Thumbnail.LoadSynchronous();
		UTexture2D* LoadLayer = Layer.LoadSynchronous();
		NewMaterial->SetTextureParameterValue("Icon", LoadThumbnail);
		...
		NewMaterial->SetTextureParameterValue("Layer", LoadLayer);
	...
	FIntPoint GetFixedDimensions() const
	{
		return Dimensions;
	}
	...
	TSoftObjectPtr<USkeletalMesh>& GetSkeletalMesh()
	{
		return SkeletalMesh;
	}
