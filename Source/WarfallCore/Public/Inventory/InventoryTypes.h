...
UE_DECLARE_GAMEPLAY_TAG_EXTERN(TAG_Consumable);
UE_DECLARE_GAMEPLAY_TAG_EXTERN(TAG_Ammunition);
...
struct FContainerMeta
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Rules") bool bFiltersEditable = true;
...
namespace WfInv
{
    ...
    inline bool  RowIsPerishable(const FItemRow& R)  { return R.CanPerish(); }
    inline int32 RowPerishSeconds(const FItemRow& R) { return R.GetPerishTime(); }
    inline float RowDurabilityMax(const FItemRow& R) { return R.MaxDurability; }
    inline float RowWearMax(const FItemRow& R)       { return R.MaxWear; }
	inline int32 RowMaxStackSize(const FItemRow& R)  { return R.MaxStackSize; }
    inline FIntPoint RowFootprint(const FItemRow& R) { return R.Thumbnail.GetFixedDimensions(); }
    inline float RowUnitMass(const FItemRow& R)      { return R.WeightConfig.GetMass(); }

	inline int32 Surface(const FIntPoint& FP) { return FMath::Max(1, FP.X) * FMath::Max(1, FP.Y); }

	inline bool PassesFilters(const FContainerMeta& Meta, const FItemRow& Row)
    {
	    if (Meta.Whitelist.Num() > 0 && !Row.Tags.HasAny(Meta.Whitelist)) return false;
    	if (Meta.Blacklist.Num() > 0 && Row.Tags.HasAny(Meta.Blacklist)) return false;
	    return true;
    }

	inline FIntPoint Rotated(const FIntPoint& FP, bool bRotate) { return bRotate ? FIntPoint(FP.Y, FP.X) : FP; }
