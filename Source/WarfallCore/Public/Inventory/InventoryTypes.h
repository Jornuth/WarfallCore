...
enum class EItemType : uint8
{
	...
	E_Other			= 1 << 5 UMETA(DisplayName = "Other")
};
...
struct FItemRow
{
	 ...
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Durability", DisplayName = "TimeMultiplier", meta = (ClampMin = 0.0f))
	...
	bool CanPerish() const { return GetConsumableVisible() && HasFlag(EItemFlags::E_CanPerished); }
	...
	float GetPerishTime() const { return PerishRate * DefaultPerishTime(); }
	float GetRepairTime() const { return RepairTimeMultiplier * DefaultRepairTime(); }
