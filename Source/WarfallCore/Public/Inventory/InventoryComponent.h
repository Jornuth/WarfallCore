...
class WARFALLCORE_API UInventoryComponent : public UActorComponent
{
	...
	DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnInventoryDirty);
...
public:
	...
	UFUNCTION(Server, Reliable) void ServerMergeAll();
	UFUNCTION(Server, Reliable) void ServerAutoArrangeAll();
	UFUNCTION(Server, Reliable, BlueprintCallable, Category="Inventory|UI") void ServerMoveByIndex(const FString& From, int32 Index, const FString& To, int32 X, int32 Y, bool bRotate);
	UFUNCTION(Server, Reliable, BlueprintCallable, Category="Inventory|UI") void ServerMoveAutoByIndex(const FString& From, int32 Index, const FString& To);
	UFUNCTION(Server, Reliable, BlueprintCallable, Category="Inventory|UI") void ServerSplitByIndex(const FString& From, int32 Index, int32 Amount);
	UFUNCTION(Server, Reliable, BlueprintCallable, Category="Inventory|UI") void ServerMergeByIndex(const FString& From, int32 IndexFrom, const FString& To, int32 IndexTo);

	// === UI wrappers (server-authoritative) ===
	UFUNCTION(Server, Reliable, BlueprintCallable, Category="Inventory|UI") void ServerRequestMergeAll();
	UFUNCTION(Server, Reliable, BlueprintCallable, Category="Inventory|UI") void ServerRequestAutoArrangeAll();
	...
	// DEBUG ONLY (server-authoritative)
	UFUNCTION(Server, Reliable) void ServerDebug_PerishIn(const FString& From, int32 Index, int32 Seconds);
	UFUNCTION(Server, Reliable) void ServerDebug_PerishNow(const FString& From, int32 Index);

	UFUNCTION(Server, Reliable)
	void ServerDebug_Setup(int32 PocketW, int32 PocketH, float PerishMult, bool bEditable=true);
...
public:	
	...
	/**
	 * Event triggered whenever the inventory state is modified.
	 *
	 * - This delegate is designed to notify listeners about any changes to the inventory,
	 *   including item additions, removals, or updates.
	 * - It can be bound to dynamically, allowing external systems to respond to inventory changes.
	 * - Usable within Blueprints for convenient integration with UI or gameplay mechanics.
	 *
	 * Attributes:
	 * - BlueprintAssignable: Enables this event to be assigned or listened to in Blueprints.
	 * - Category = "Inventory|Events": Groups the event under the specified category in the editor.
	 */
	UPROPERTY(BlueprintAssignable, Category="Inventory|Events")
	FOnInventoryDirty OnInventoryDirty;

public: // Temp -> Private
	UPROPERTY(ReplicatedUsing=OnRep_Revision)
	int32 ReplicationRevision = 0;
