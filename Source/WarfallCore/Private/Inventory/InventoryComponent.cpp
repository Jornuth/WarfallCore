...
void UInventoryComponent::ServerRequestMoveItem_Implementation(const FGuid& InstanceGuid, const FContainerId& From,
	const FContainerId& To, const FGridCoord& ToPos, bool bRotate)
{
	// Résoudre containers
	FContainerData* FromData = nullptr; FContainerMeta* FromMeta = nullptr;
	FContainerData* ToData   = nullptr; FContainerMeta* ToMeta   = nullptr;
	...
	FItemInstance Moving = Entry->Instance;
	const FIntPoint FP = Entry->FootprintPlaced;

	MarkRemoved(*FromData, FromIndex);

	if (ToData == FromData && !bAuto)
	{
		// Move/rotate exact au sein du même container
		const FIntPoint TargetSize = bRotate ? FIntPoint(FP.Y, FP.X) : FP;
		if (FitsAt(*ToData, *ToMeta, ToPos, TargetSize))
		{
			FContainerEntry NewE;
			NewE.Instance = Moving;
			NewE.TopLeft = ToPos;
			NewE.bRotated90 = bRotate;
			NewE.FootprintPlaced = TargetSize;
			ToData->Entries.Add(NewE);
			MarkAdded(*ToData, ToData->Entries.Last());
			return;
		}
		// Échec: remettre à sa place d'origine (fallback)
		FContainerEntry Back;
		Back.Instance = Moving;
		Back.TopLeft = Entry->TopLeft;
		Back.bRotated90 = Entry->bRotated90;
		Back.FootprintPlaced = FP;
		FromData->Entries.Add(Back);
		MarkAdded(*FromData, FromData->Entries.Last());
		return;
	}
    // Move vers autre container OU auto-place
    // Si position exacte demandée
    if (!bAuto)
    {
        // Taille cible en fonction de 'bRotate' (on relit la Row pour footprint canonique)
        UDataTable* ItemsTable = GetItemsDT();
        const FItemRowDetail* Row = ItemsTable ? ItemsTable->FindRow<FItemRowDetail>(Moving.RowID, TEXT("MoveExact")) : nullptr;
        const FIntPoint Canon = Row ? RowFootprint(Row->Details) : FP;
        const FIntPoint TargetSize = bRotate ? FIntPoint(Canon.Y, Canon.X) : Canon;
        ...
        // Échec: remettre dans From au même endroit
        FContainerEntry Back;
        Back.Instance = Moving;
        Back.TopLeft = Entry->TopLeft;
        Back.bRotated90 = Entry->bRotated90;
    ...
    // Auto-place dans To (merge d'abord, puis nouveaux stacks)
    UDataTable* ItemsTable = GetItemsDT();
    const FItemRowDetail* Row = ItemsTable ? ItemsTable->FindRow<FItemRowDetail>(Moving.RowID, TEXT("MoveAuto")) : nullptr;
    if (!Row)
    {
        // Fallback: remettre
        FContainerEntry Back;
    ...
    // On essaie d'abord de merge dans To
    FItemInstance Temp = Moving;
    if (TryMergeIntoContainer(Temp, Row->Details, *ToData, *ToMeta))
    {
        if (Temp.Count <= 0) return; // tout absorbé
    }
    // Puis placement en nouveaux stacks (ici, Count est petit : 1 stack)
    const int32 Placed = PlaceNewStacks(Temp, Row->Details, *ToData, *ToMeta);
    if (Placed > 0) return;

    // Rien n’a marché -> remettre dans From à sa place d’origine
    FContainerEntry Back;
    Back.Instance = Moving;
    Back.TopLeft = Entry->TopLeft;
    Back.bRotated90 = Entry->bRotated90;
	...
	const FContainerMeta& Meta)
{
	...
	while (Instance.Count > 0)
	{
		...
		if (NewStack.Perish.NextPerishEpoch > 0 && Meta.PerishRateMultiplier != 1.f)
		{
			...
			const int64 Adjust = (int64)FMath::RoundToInt((double)Remain / (double)M);
...
void UInventoryComponent::BuildAsciiGrid(const FContainerData& Data, const FContainerMeta& Meta,
                                         TArray<FString>& OutLines) const
{
	const int32 W = Meta.Grid.Width;
	const int32 H = Meta.Grid.Height;
	if (W <= 0 || H <= 0) { OutLines.Reset(); return; }
	...
	auto Put = [&](int32 X, int32 Y, TCHAR C)
	{
		if (X >= 0 && Y >= 0 && X < W && Y < H)
			Buf[Y*W + X] = C;
	};
	...
	for (const FContainerEntry& E : Data.Entries)
	{
		// Choose a letter from RowID to paint the footprint
		...
		// Mark top-left with '*' to spot stack origins quickly
	...
	for (int32 y = 0; y < H; ++y)
	{
		FString Line; Line.Reserve(W + 2);
		for (int32 x = 0; x < W; ++x)
		{
			Line.AppendChar(Buf[y*W + x]);
		}
		OutLines.Add(Line);
	}
}
...
{
	Data.Entries.RemoveAtSwap(Index);
	Data.MarkArrayDirty();
}
...
void UInventoryComponent::ServerMoveByIndex(const FString& From, int32 Index, const FString& To, int32 X, int32 Y, bool bRotate)
{
...
void UInventoryComponent::ServerMoveAutoByIndex(const FString& From, int32 Index, const FString& To)
{
    FContainerId FromId, ToId; FContainerData* FromData=nullptr; FContainerData* ToData=nullptr;
    if (!UI_ParseContainerId(this, From, FromId, FromData) || !UI_ParseContainerId(this, To, ToId, ToData)) return;
    if (!FromData || !FromData->Entries.IsValidIndex(Index)) return;
...
void UInventoryComponent::ServerSplitByIndex(const FString& From, int32 Index, int32 Amount)
{
...
void UInventoryComponent::ServerMergeByIndex(const FString& From, int32 IndexFrom, const FString& To, int32 IndexTo)
{
    FContainerId FromId; FContainerData* FromData=nullptr;
