...
UE_DEFINE_GAMEPLAY_TAG(TAG_Consumable, "Item.Consumable");
UE_DEFINE_GAMEPLAY_TAG(TAG_Ammunition, "Item.Ammunition");
...
void FCustomItemRow::DeclareProperties()
{
	...
	TagsHandle = PropertyHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FItemRow, Tags));
...
void FCustomItemRow::OnTypeChanged()
{
	if (CurrentType == Handle->Type)
		return;
	Handle->Tags.RemoveTag(TAG_Consumable);
	Handle->Tags.RemoveTag(TAG_Ammunition);
	if (Handle->GetConsumableVisible()) Handle->Tags.AddTag(TAG_Consumable);
	if (Handle->Type == EItemType::E_Ammunition) Handle->Tags.AddTag(TAG_Ammunition);
	PropertyUtils->ForceRefresh();
	CurrentType = Handle->Type;
}
