...
UE_DEFINE_GAMEPLAY_TAG(ITEM_Ammunition, "Item.Ammunition");
UE_DEFINE_GAMEPLAY_TAG(ITEM_Ingredient, "Item.Ingredient");

// ===============================[ Item.State GameplayTags ]============================
UE_DEFINE_GAMEPLAY_TAG(ITEM_STATE_Lootable, "Item.State.Lootable");
UE_DEFINE_GAMEPLAY_TAG(ITEM_STATE_Perishable, "Item.State.Perishable");
UE_DEFINE_GAMEPLAY_TAG(ITEM_STATE_Repairable, "Item.State.Repairable");
UE_DEFINE_GAMEPLAY_TAG(ITEM_STATE_Dismantleable, "Item.State.Dismantleable");
UE_DEFINE_GAMEPLAY_TAG(ITEM_STATE_Container, "Item.State.Container");

// ========================[ Item.Meta.Weapon GameplayTags ]========================
UE_DEFINE_GAMEPLAY_TAG(ITEM_META_WEAPON_Damage, "Item.Meta.Weapon.Damage");
UE_DEFINE_GAMEPLAY_TAG(ITEM_META_WEAPON_AttackSpeed, "Item.Meta.Weapon.AttackSpeed");

// ========================[ Item.Meta.Armor GameplayTags ]========================
UE_DEFINE_GAMEPLAY_TAG(ITEM_META_ARMOR_ArmorValue, "Item.Meta.Armor.ArmorValue");
...
UE_DEFINE_GAMEPLAY_TAG(ITEM_META_UTILS_MaxDurability, "Item.Meta.Utils.MaxDurability");
UE_DEFINE_GAMEPLAY_TAG(ITEM_META_UTILS_MaxWear, "Item.Meta.Utils.MaxWear");
UE_DEFINE_GAMEPLAY_TAG(ITEM_META_UTILS_Weight, "Item.Meta.Utils.Weight");
UE_DEFINE_GAMEPLAY_TAG(ITEM_META_UTILS_Capacity, "Item.Meta.Utils.Capacity");

// ===============================[ Ingredients.Ressource GameplayTags ]============================
UE_DEFINE_GAMEPLAY_TAG(INGREDIENTS_RESSOURCES_WoodLog, "Ingredients.Ressources.WoodLog");

// ===============================[ Ingredients.Component GameplayTags ]============================
UE_DEFINE_GAMEPLAY_TAG(INGREDIENTS_COMPONENTS_MetalBar, "Ingredients.Components.MetalBar");
...
FItemRow::FItemRow()
{
	...
	DropMesh = nullptr;
	Flags.AddTag(ITEM_STATE_Lootable);
	Flags.AddTag(ITEM_STATE_Perishable);
	Flags.AddTag(ITEM_STATE_Repairable);
                                       ...
                                       IDetailChildrenBuilder& ChildBuilder, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	...
	if (Handle->Type == EItemType::E_Ingredient)
	{
		IDetailGroup& IngredientsGroup = ChildBuilder.AddGroup("Ingredients", FText::FromString("Ingredients"));
		IngredientsGroup.AddPropertyRow(IngredientsTypeHandle.ToSharedRef());
		IngredientsGroup.AddPropertyRow(MetaModifiersHandle.ToSharedRef());
	}
...
void FCustomItemRow::DeclareProperties()
{
	...
	IngredientsTypeHandle = PropertyHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FItemRow, IngredientsType));
	MetaModifiersHandle = PropertyHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FItemRow, MetaModifiers));
...
void FCustomItemRow::OnTypeChanged()
{
	if (CurrentType == Handle->Type)
		return;
	Handle->Tags.RemoveTag(ITEM_Consumable);
	Handle->Tags.RemoveTag(ITEM_Ammunition);
	Handle->Tags.RemoveTag(ITEM_Ingredient);
	if (Handle->GetConsumableVisible()) Handle->Tags.AddTag(ITEM_Consumable);
	if (Handle->Type == EItemType::E_Ammunition) Handle->Tags.AddTag(ITEM_Ammunition);
	if (Handle->Type == EItemType::E_Ingredient) Handle->Tags.AddTag(ITEM_Ingredient);
	PropertyUtils->ForceRefresh();
	CurrentType = Handle->Type;
}
