...
void AWarfallCharacter::iv_p2_setup(int32 PocketW, int32 PocketH, float PerishMult)
{
    ...
    {
        // Reset containers
        Inv->Backpack.Entries.Reset();
        Inv->Pockets.Reset();
        Inv->PocketsMeta.Reset();

        // Backpack base size (ajuste à ton défaut)
        Inv->BackpackMeta.Grid = { 8, 6 };
        Inv->BackpackMeta.PerishRateMultiplier = 1.f;

        // 1 Pocket Food éditable pour test
        FContainerData PocketData;
        Inv->Pockets.Add(PocketData);

        FContainerMeta Meta;
        Meta.Grid = { PocketW, PocketH };
        Meta.PerishRateMultiplier = PerishMult;
        Meta.bFiltersEditable = true; // copie de Row.Pocket.bPlayerEditableFilters IRL
        Inv->PocketsMeta.Add(Meta);

        IVLOG(TEXT("[P2_SETUP %s] Backpack=%dx%d  Pocket0=%dx%d  PerishMult=%.2f (editable)"),
            SideTag(this),
            Inv->BackpackMeta.Grid.Width, Inv->BackpackMeta.Grid.Height,
            Inv->PocketsMeta[0].Grid.Width, Inv->PocketsMeta[0].Grid.Height,
            Inv->PocketsMeta[0].PerishRateMultiplier);
    }
}
...
void AWarfallCharacter::iv_p2_arrange()
{
    ...
    {
        Inv->AutoArrangeAll();
        IVLOG(TEXT("[ARRANGE %s] Done"), SideTag(this));
    }
}
...
void AWarfallCharacter::iv_p2_overlay(const FString& Which)
{
    if (auto* Inv = GetInv(this))
    {
        ...
        {
            Inv->BuildAsciiGrid(Inv->Backpack, Inv->BackpackMeta, Lines);
        ...
        else if (Which.Len() >= 2 && (Which[0] == 'p' || Which[0] == 'P'))
        {
            ...
            if (Inv->Pockets.IsValidIndex(Index))
            {
                Inv->BuildAsciiGrid(Inv->Pockets[Index], Inv->PocketsMeta[Index], Lines);
