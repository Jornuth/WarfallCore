...
void AThumbnailMaker::UpdateMesh() const
{
	if (ThumbnailRessource)
	{
		...
		{
			SkeletalObject->SetSkeletalMeshAsset(ThumbnailRessource->GetSkeletalMesh());
			return;
		}
		if (ThumbnailRessource->GetMesh())
		{
			MeshObject->SetStaticMesh(ThumbnailRessource->GetMesh());
			SkeletalObject->SetSkeletalMeshAsset(nullptr);
			Refresh();
		}
	}
...
void AThumbnailMaker::ImportAndAssignTextureToRow(const FString& FileName) const
{
	...
	const FString PackagePath = THUMBNAIL_FOLDER_PATH;
	...
	// Supprimer asset existant proprement
	if (UObject* Existing = StaticFindObject(UObject::StaticClass(), Package, *FileName))
	{
		const TArray<UObject*> AssetsToDelete = { Existing };
	...
	if (!Imported)
	{
		UE_LOG(LogTemp, Error, TEXT("Failed to import texture manually: %s"), *SourceImagePath);
		return;
	}

	UTexture2D* FinalTexture = Cast<UTexture2D>(Imported);
	if (!FinalTexture)
	{
		UE_LOG(LogTemp, Error, TEXT("Imported asset is not a UTexture2D."));
		return;
	}

	// Enregistrer proprement
	if (FinalTexture->MarkPackageDirty())
	...
	if (const FString FinalPackagePath = FPackageName::LongPackageNameToFilename(PackageName, FPackageName::GetAssetPackageExtension());
		!UPackage::SavePackage(Package, FinalTexture, *FinalPackagePath, SaveArgs))
	{
		UE_LOG(LogTemp, Warning, TEXT("Failed to save package to disk: %s"), *FinalPackagePath);
	}
	else
	{
		UE_LOG(LogTemp, Log, TEXT("Successfully saved asset package: %s"), *FinalPackagePath);
	}

	// Mise à jour de la structure
	if (ThumbnailRessource)
	{
		ThumbnailRessource->Thumbnail = FinalTexture;
	}		
}